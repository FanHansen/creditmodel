...
xgb_filter <- function(dat_train, dat_test = NULL, target = "flag", pos_flag = NULL,
    ...
    if (!is.null(cv_folds) && cv_folds > 1) {
        ...
        occur_time = occur_time, seed = 46, note = FALSE, save_data = TRUE)
        dat_train = train_test$train
        dat_test = train_test$test
        k = 1
    }
...
gbm_filter <- function(dat, target = NULL, x_list = NULL, ex_cols = NULL, pos_flag = NULL,
                       ...
                       cores_num = 2, vars_name = TRUE, note = TRUE, save_data = TRUE,
                       file_name = NULL,
...
#' save_data = TRUE, vars_name = FALSE)
...
fast_high_cor_filter <- function(dat, p = 0.7, x_list = NULL, com_list = NULL,
                                 ex_cols = NULL, save_data = TRUE, cor_class = TRUE,
...
dir_path = tempdir(), save_data = TRUE, note = FALSE, ...) {
...
lasso_filter <- function(dat_train, dat_test = NULL, target = NULL,
                         ...
                         save_data = TRUE, plot.it = TRUE, seed = 46,
                         file_name = NULL, dir_path = tempdir()) {
    ...
    if (is.null(dat_test)) {
        train_test = train_test_split(dat_ts, split_type = "Random", prop = 0.3,
                                      seed = 46, note = FALSE, save_data = TRUE)
    } else {
        nr = nrow(dat_train)
        train_test = train_test_split(dat_ts, split_type = "byRow", prop = nr / nrow(dat_ts),
                                      seed = 46, note = FALSE, save_data = TRUE)
...
#' @importFrom gridExtra arrangeGrob 
#' @export
get_auc_ks_lambda <- function(lasso_model, x_test, y_test, save_data = TRUE, plot_show = TRUE, file_name = NULL, dir_path = tempdir()) {
