...
training_model <- function(model_name = "mymodel", dat, dat_test = NULL,
    ...
    if (length(unique(dat_train[, target])) == 2) {
        if (any(algorithm == "LR")) {
            ...
            if (score_card) {
                ...
                test_score$score_LR = score_transfer(model = lr_model_new,
                                                   tbl_woe = test_woe,
                                                   save_data = TRUE)[, "score"]
                save_dt(train_score, file_name = paste(model_name, "LR.score.train", sep = "."),
                    dir_path = LR_pred_dir_path, note = TRUE)
                save_dt(test_score, file_name = paste(model_name, "LR.score.test", sep = "."),
                    dir_path = LR_pred_dir_path, note = TRUE)
            }
            ...
            if (isTRUE(all.equal(names(train_pred), names(test_pred))) &
                ...
                ggsave(filename = paste0(LR_perf_dir_path, "/", paste(paste0(gtitle, ".ks_plot"), "png", sep = '.')), device = "png",
                   plot = ks_pl, dpi = "retina", width = 5, height = 4.5)
                ggsave(filename = paste0(LR_perf_dir_path, "/", paste(paste0(gtitle, ".roc_plot"), "png", sep = '.')), device = "png",
                    plot = roc_pl, dpi = "retina", width = 5, height = 4.5)
                ggsave(filename = paste0(LR_perf_dir_path, "/", paste(paste0(gtitle, ".psi_plot"), "png", sep = '.')), device = "png",
                         plot = psi_pl, dpi = "retina", width = 5, height = 4.5)
                ggsave(paste0(LR_perf_dir_path, "/", paste(paste0(gtitle, ".perf_table"), "png", sep = '.')),
                    arrangeGrob(grobs = list(tb_ks), ncol = 1, nrow = 1), dpi = "retina", width = 12)
                save_dt(perf_tb, file_name = paste0(gtitle, ".perf_table"), dir_path = LR_perf_dir_path)
                ggsave(filename = paste0(LR_perf_dir_path, "/", paste(paste0(gtitle, ".lift_plot"), "png", sep = '.')), device = "png",
        ...
        if (any(algorithm == "XGB")) {
			...
			save_dt(xgb_train, file_name = "train_XGB", dir_path = XGB_data_dir_path, note = TRUE)
            save_dt(xgb_test, file_name = "test_XGB", dir_path = XGB_data_dir_path, note = TRUE)
            # Train a model
            if (!is.null(seed)) set.seed(seed) else set.seed(46)
            ...
            save_dt(train_pred, file_name = "train_pred_XGB", dir_path = XGB_pred_dir_path, note = TRUE)
            save_dt(test_pred, file_name = "test_pred_XGB", dir_path = XGB_pred_dir_path, note = TRUE)
            if (isTRUE(all.equal(names(train_pred), names(test_pred)))) {
                ...
                ggsave(filename = paste0(XGB_perf_dir_path, "/", paste(paste0(gtitle, ".ks_plot"), "png", sep = '.')), device = "png",
                   plot = ks_pl, dpi = "retina", width = 5, height = 4.5)
                ggsave(filename = paste0(XGB_perf_dir_path, "/", paste(paste0(gtitle, ".roc_plot"), "png", sep = '.')), device = "png",
                    plot = roc_pl, dpi = "retina", width = 5, height = 4.5)
                ggsave(filename = paste0(XGB_perf_dir_path, "/", paste(paste0(gtitle, ".psi_plot"), "png", sep = '.')), device = "png",
                         plot = psi_pl, dpi = "retina", width = 5, height = 4.5)
                ggsave(paste0(XGB_perf_dir_path, "/", paste(paste0(gtitle, ".perf_table"), "png", sep = '.')),
                    arrangeGrob(grobs = list(tb_ks), ncol = 1, nrow = 1), dpi = "retina", width = 12)
                save_dt(perf_tb, file_name = paste0(gtitle, ".perf_table"), dir_path = XGB_perf_dir_path, note = TRUE)
                ggsave(filename = paste0(XGB_perf_dir_path, "/", paste(paste0(gtitle, ".lift_plot"), "png", sep = '.')), device = "png",
                   plot = lift_pl, dpi = "retina", width = 5, height = 4.5)
        ...
        if (any(algorithm == "GBM")) {
            ...
            if (isTRUE(all.equal(names(train_pred), names(test_pred)))) {
               ...
               perf_tb = perf_table(train_pred = train_pred, test_pred = test_pred, target = target, score = "prob_GBM", total = TRUE, pos_flag = pos_flag,g = 20)
                ...
                ggsave(filename = paste0(GBM_perf_dir_path, "/", paste(paste0(gtitle, ".ks_plot"), "png", sep = '.')), device = "png",
                   plot = ks_pl, dpi = "retina", width = 5, height = 4.5)
                ggsave(filename = paste0(GBM_perf_dir_path, "/", paste(paste0(gtitle, ".roc_plot"), "png", sep = '.')), device = "png",
                    plot = roc_pl, dpi = "retina", width = 5, height = 4.5)
                ggsave(filename = paste0(GBM_perf_dir_path, "/", paste(paste0(gtitle, ".psi_plot"), "png", sep = '.')), device = "png",
                         plot = psi_pl, dpi = "retina", width = 5, height = 4.5)
                ggsave(paste0(GBM_perf_dir_path, "/", paste(paste0(gtitle, ".perf_table"), "png", sep = '.')),
                    arrangeGrob(grobs = list(tb_ks), ncol = 1, nrow = 1), dpi = "retina", width = 12)
                save_dt(perf_tb, file_name = paste0(gtitle, ".perf_table"), dir_path = GBM_perf_dir_path,note = TRUE)
                ggsave(filename = paste0(GBM_perf_dir_path, "/", paste(paste0(gtitle, ".lift_plot"), "png", sep = '.')), device = "png",
                   plot = lift_pl, dpi = "retina", width = 5, height = 4.5)
        ...
        if (any(algorithm == "RF")) {
            ...
            if (isTRUE(all.equal(names(train_pred), names(test_pred)))) {
                ...
                ggsave(filename = paste0(RF_perf_dir_path, "/", paste(paste0(gtitle, ".ks_plot"), "png", sep = '.')), device = "png",
                   plot = ks_pl, dpi = "retina", width = 5, height = 4.5)
                ggsave(filename = paste0(RF_perf_dir_path, "/", paste(paste0(gtitle, ".roc_plot"), "png", sep = '.')), device = "png",
                    plot = roc_pl, dpi = "retina", width = 5, height = 4.5)
                ggsave(filename = paste0(RF_perf_dir_path, "/", paste(paste0(gtitle, ".psi_plot"), "png", sep = '.')), device = "png",
                         plot = psi_pl, dpi = "retina", width = 5, height = 4.5)
                ggsave(paste0(RF_perf_dir_path, "/", paste(paste0(gtitle, ".perf_table"), "png", sep = '.')),
                    arrangeGrob(grobs = list(tb_ks), ncol = 1, nrow = 1), dpi = "retina", width = 12)
                save_dt(perf_tb, file_name = paste0(gtitle, ".perf_table"), dir_path = RF_perf_dir_path,note = TRUE)
                ggsave(filename = paste0(RF_perf_dir_path, "/", paste(paste0(gtitle, ".lift_plot"), "png", sep = '.')), device = "png",
                   plot = lift_pl, dpi = "retina", width = 5, height = 4.5)
...
#' pred_score
#'
#' \code{pred_score} is for using trained models to predict new data.
#' @param model  Model generated by \code{\link{training_model}}.
