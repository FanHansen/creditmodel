...
#'
#' @examples
#' train_test <- train_test_split(UCICreditCard, split_type = "Random", prop = 0.8, save_data = TRUE)
...
#' ex_cols = "ID|apply_date", save_data = TRUE)
...
bins_total = FALSE, best = TRUE, g = 10, as_table = TRUE, note = FALSE, bins_no = FALSE) {
    ...
    if (is.null(dat_test)) {
        ...
        occur_time = occur_time, save_data = TRUE, note = FALSE)
...
get_psi <- function(dat, x, dat_test = NULL, occur_time = NULL, start_date = NULL, cut_date = NULL,
 ...
 as_table = TRUE, note = FALSE, bins_no = TRUE) {
    ...
    if (is.null(dat_test)) {
        ...
        start_date = start_date, cut_date = cut_date, save_data = TRUE, note = FALSE)
        dfe = df_ae$train
        dfa = df_ae$test
        dfa$ae = "actual"
        dfe$ae = "expected"
