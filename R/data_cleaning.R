...
#' @param file_name  The name for periodically saved data file. Default is "dat_cl".
...
#' @examples
#' \dontrun{
#' dat_cl = cleaning_data(dat = lendingclub, target = "loan_status", x_list = NULL, 
#' obs_id = "id", occur_time = "issue_d",
#' ex_cols = c("_d$|pymnt|out_prncp_inv|recoveries|funded_amnt|total_rec_",
#' "collection_recovery_fee"), 
#' save_data = FALSE, outlier_proc = TRUE, missing_proc = TRUE, 
#' default_miss = FASLE, low_var = TRUE, dir_path = "./data", file_name = "dat_cl")
#' }
#'
...
#' @export
cleaning_data <- function(dat, target = NULL, x_list = NULL, obs_id = NULL, occur_time = NULL, pos_flag = NULL, miss_values = NULL,
ex_cols = NULL, save_data = FALSE, parallel = FALSE, outlier_proc = TRUE, note = TRUE,
missing_proc = TRUE, default_miss = TRUE, low_var = TRUE, dir_path = "./data", file_name = "dat_cl") {
    ...
    if (length(dat) > 0) {
        ...
        dat = checking_data(dat = dat, target = target, pos_flag = pos_flag, occur_time = occur_time, note = note) %>%
        ...
        time_transfer(date_cols = NULL, ex_cols = c(obs_id, target, occur_time, ex_cols), note = note) %>%
        ...
        merge_category(ex_cols = c(obs_id, target, occur_time, ex_cols), p = 0.001, m = 20, note = note) %>%
        remove_duplicated(target = target, obs_id = obs_id, occur_time = occur_time, note = note)
 
        char_x_list = get_names(dat = dat, types = c('factor', 'character'), ex_cols = c(obs_id, target, occur_time, ex_cols), get_ex = FALSE)
        num_x_list = get_names(dat = dat, types = c('numeric', 'integer', 'double'), ex_cols = c(obs_id, target, occur_time, ex_cols), get_ex = FALSE)
        date_x_list = get_names(dat = dat, types = c("Date", "POSIXlt", "POSIXct", "POSIXt"), ex_cols = c(obs_id, target, occur_time, ex_cols), get_ex = FALSE)
        ex_x_list = get_names(dat = dat, types = c('logical', 'factor', 'character', 'numeric', 'integer', 'double', "Date", "POSIXlt", "POSIXct", "POSIXt"), ex_cols = ex_cols, get_ex = TRUE)
        flag_list = get_names(dat = dat, types = c('logical', 'factor', 'character', 'numeric', 'integer', 'double', "Date", "POSIXlt", "POSIXct", "POSIXt"), ex_cols = c(obs_id, occur_time, target), get_ex = TRUE)
        ...
        if (outlier_proc) {
            dat = process_outliers(dat = dat, target = target, x_list = num_x_list, ex_cols = c(obs_id, occur_time, target), parallel = parallel, note = note)
        }
        if (missing_proc) {
            dat = process_nas(dat = dat, class_var = FALSE, x_list = c(num_x_list, char_x_list), ex_cols = c(obs_id, occur_time, target), default_miss = default_miss, parallel = parallel, method = "median", note = note)
        } 
        if (save_data) {
            save_dt(dat, dir_path = dir_path, file_name = file_name, append = FALSE, note = note)
        }
    }
...
remove_duplicated <- function(dat = dat, obs_id = NULL, occur_time = NULL, target = NULL , note = TRUE) {
...
#'
#' @examples
#' datss = null_blank_na(dat = UCICreditCard, miss_values =list(-1,-2))
#' @export
#' @importFrom dplyr group_by mutate summarize  summarise n  count %>% filter mutate_if
null_blank_na <- function(dat, miss_values = NULL, note = TRUE) {
...
#'
#' @examples
#' datss = entry_rate_max(dat = UCICreditCard , rt  = 0.98)
#' @export

entry_rate_max <- function(dat, rt = 0.95, note = TRUE) {
    if (note) cat("[NOTE]", "process unique value which rate is  more than 95% \n")
    n_row <- nrow(dat)
    max_rate = vapply(dat, function(x) max(table(x, useNA = "no")) / n_row, FUN.VALUE = numeric(1))
    dat[which(max_rate > rt)] <- vapply(dat[which(max_rate > rt)], function(x) {
        ifelse(is.na(x), NA, ifelse(as.character(x) == names(table(x, useNA = "no")[which.max(table(x, useNA = "no"))]), 1, 0))
...
entry_rate_na <- function(dat, nr = 0.95, note = TRUE) {
...
#' @examples
#' \dontrun{
#' dat = low_variance_filter(lendingclub, lvp = 0.9)
#' }
#' @export
low_variance_filter <- function(dat, lvp = 0.97, note = FALSE) {
    ...
    low_variance <- vapply(dat, function(x) max(table(x, useNA = "always")) / nrow(dat), FUN.VALUE = numeric(1))
